...
<body onload="fetchData()"> <!-- เรียก fetchData เมื่อหน้าโหลด -->
    ...
    <script>
        function fetchData() {
            fetch('showdata.php')
                ...
                .then(data => {
                    document.getElementById('card_id').textContent = data.card_id || 'Not Found';               // แสดง card_id          // แสดง distance
                    document.getElementById('user_license_plate').textContent = data.user_license_plate || 'Not Found';  // แสดง user_license_plate
                    document.getElementById('timestamp').textContent = data.timestamp || 'Not Found';                // แสดง timestamp
                })
