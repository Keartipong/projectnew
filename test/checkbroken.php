<?php
// PHP Code for handling database connection and query
if (isset($_GET['zone'])) {
    $zone = $_GET['zone'];
    ...
    $sql = "SELECT number, status FROM lots WHERE zone = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $zone);
    $stmt->execute();
    $result = $stmt->get_result();

    // ดึงข้อมูลและส่งเป็น JSON
...
<body>
    ...
    <div class="legend">
        <div class="legend-item">
            <div class="legend-color vacant"></div> : ปกติ
        </div>
        <div class="legend-item">
            <div class="legend-color reserved"></div> : จอด
        </div>
        <div class="legend-item">
            <div class="legend-color occupied"></div> : เสีย
        </div>
    </div>
    ...
    fetch(`path_to_your_php_script.php?zone=${zone}`)
        ...
        .then(lots => {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                for (let j = 0; j < 4; j++) {
                    const lotIndex = i * 4 + j;
                    if (lotIndex < lots.length) {
                        lotsHtml += `<div class="lot" style="background-color: ${getColor(lots[lotIndex].status)}">${lots[lotIndex].number}</div>`;
                    } else {
                        lotsHtml += `<div class="lot" style="background-color: #e0e0e0"></div>`; // ช่องจอดว่างเปล่า
                    }
                }
                lotsHtml += `</div>`;
            }
            ...
            for (let i = 24; i < 29; i++) {
                if (i < lots.length) {
                    ...
                    lotsHtml += `<div class="lot lot-5th" style="background-color: #e0e0e0"></div>`; // ช่องจอดว่างเปล่า
                }
            }
            lotsHtml += `</div>`;
...
function getColor(status) {
    switch (status) {
        ...
        case 2: return "#64b5f6"; // จอง
        case 3: return "#FF0033"; // เสีย
